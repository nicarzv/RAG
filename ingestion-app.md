...
|-----|-----------------|--------------|
| **SharePoint indexer** | Configurable | Crawl SharePoint sites defined in Cosmos DB `datasources` |
| **SharePoint purger** | 10 min after indexer | Remove index entries for deleted SharePoint items |
| **Blob indexer** | `0 * * * *` (hourly) | Crawl the `documents` blob container |
| **Blob purger** | `10 * * * *` (10 min after) | Remove index entries for deleted blobs |
| **NL2SQL indexer** | Configurable | Index SQL table schemas and sample queries |
| **NL2SQL purger** | Configurable | Clean up stale NL2SQL entries |
| **Image purger** | Configurable | Remove orphaned extracted images |

### 3.2 Important: Both Indexers Are Always Active

Both the SharePoint indexer and Blob indexer are registered as cron jobs from the moment the app starts. Neither is "disabled" by default. The practical difference:
...
There is no `USE_SHAREPOINT=true` or `ENABLE_BLOB_INDEXER=false` flag. Both run, both check for work, and both silently exit if there's nothing to process.
...
**Structured App Insights logs** via `_log_event()` — JSON payloads with event names like `RUN-START`, `ITEM-COMPLETE`, `RUN-COMPLETE` that include all counters and can be queried via KQL in Application Insights.
...
Plus a `latest.json` pointer and `{runId}.json` detailed file.

### 13.2 Key Log Events

| Event | What It Logs |
|-------|-------------|
| `RUN-START` | Indexer name, run ID, start timestamp, site configs |
| `ITEM-COMPLETE` | Item ID, parent_id, freshness decision, chunk count, duration |
| `RUN-COMPLETE` | All aggregate counters (discovered, indexed, skipped, failed) |
| `EMBED-RETRY` | Rate limit or transient error, retry count, wait time |
| `UPLOAD-BATCH` | Batch size, success/failure count |

### 13.3 Run Summary Counters

| Counter | Meaning |
|---------|---------|
| `items_discovered` | Total items found in source |
| `items_indexed` | Items that were processed and uploaded |
