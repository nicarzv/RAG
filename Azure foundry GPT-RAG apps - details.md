...
| **Repository** | github.com/Azure/gpt-rag-orchestrator (v2.4.2) |
| **Ingress** | External (HTTPS, TLS enforced) |
| **Replicas** | min: 1, max: 1 |
| **Resources** | 0.5 vCPU, 1 GiB RAM |
| **Dapr** | Enabled (appId: `orchestrator`, port 80, HTTP) |
| **Purpose** | Receives queries from the Frontend, loads system prompts from disk files (default, `PROMPT_SOURCE=file`) or optionally from Cosmos DB, executes hybrid search against AI Search with user token propagation, calls OpenAI for grounded answers, saves conversation history |
...
| **Repository** | github.com/Azure/gpt-rag-ui (v2.2.2) |
| **Ingress** | External (HTTPS, TLS enforced) |
| **Replicas** | min: 1, max: 1 |
| **Resources** | 0.5 vCPU, 1 GiB RAM |
...
| **Repository**     | github.com/Azure/gpt-rag-ingestion (v2.2.3)                                                                                                                |
| **Ingress**        | External (HTTPS, TLS enforced)                                                                                                                             |
| **Replicas**       | min: 1, max: 1                                                                                                                                             |
| **Resources**      | 0.5 vCPU, 1 GiB RAM                                                                                                                                        |
...
- **Prompt loading:** Default is filesystem (`prompts/` directory, `PROMPT_SOURCE=file` set by Bicep). Optionally Cosmos DB (`prompts` container) if you change `PROMPT_SOURCE=cosmos` in App Configuration
...
1. **Orchestrator:** Customize system prompts in `/prompts/` files (default) or switch to Cosmos DB by setting `PROMPT_SOURCE=cosmos`, tune search parameters (`TOP_K`, `SEARCH_APPROACH`), adjust chat model settings (`TEMPERATURE`, `TOP_P`)
